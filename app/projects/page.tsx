...
const projects = [
  {
    ...
    details: [
      ...
      { label: "Корпуса", value: "3-4 корпуса, до 5 этажей" },
      ...
      { label: "Сервис", value: "Консьерж, кинорум, коворкинг" },
      { label: "Прогноз ROI", value: "40-60% за 24-36 месяцев" },
      { label: "Срок реализации", value: "24-36 месяцев" },
    ],
    ...
    <section className="py-24">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="space-y-16">
          {projects.map((p) => (
            <article
              key={p.id}
              id={p.id}
              className="scroll-mt-24 card-dark p-8 md:p-10"
            >
              ...
              <h2 className="text-2xl md:text-3xl font-semibold mb-2 text-white">{p.title}</h2>
              <p className="card-dim mb-6">{p.subtitle}</p>
              <p className="card-muted leading-relaxed mb-8 max-w-3xl">{p.description}</p>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                {p.details.map((d) => (
                  <div key={d.label} className="p-4 rounded-lg bg-white/5 border border-white/10">
                    <div className="text-xs card-dim mb-1">{d.label}</div>
                    <div className="text-sm font-medium text-white">{d.value}</div>
                  </div>
                ))}
              </div>

              <div className="p-5 rounded-lg border-l-2 border-gold bg-gold/5">
                <p className="text-sm card-muted leading-relaxed">{p.highlight}</p>
              </div>
            </article>
          ))}
        </div>
        ...
        <div className="mt-20 text-center">
          ...
          <Link
            href="/contact"
            className="inline-flex items-center justify-center px-10 py-4 bg-navy text-white font-medium text-lg rounded hover:bg-navy-light transition-colors"
          >
            Связаться
          </Link>
        </div>
